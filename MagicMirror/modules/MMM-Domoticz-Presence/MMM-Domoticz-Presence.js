 ...
 * version 1.04 15th April 2018
 * By SpoturDeal https://github.com/SpoturDeal
 * MIT Licensed.
 */
 Module.register('MMM-Domoticz-Presence', {
	defaults: {
    ...
    var text = '<div>';
        ...
        for (i=0;i<data.result.length;i++){
            ...
            if (dev.Name.indexOf('Presence') > -1) {
              if (dev.Type.indexOf('Temp') >- 1){
                ...
                power += '<tr><td class="small">' + dev.LastUpdate.substr(11,5)+'</td><td class="small"> '+dev.Name.substr(11,50) + '</td><td class="small "><i class="fa ' + icon + '"></i></td></tr>';
              }
        ...
        if (blindsCount > 0){
            text += (this.config.showItems.indexOf('blinds') !== -1?blinds:'');
        }
        if (coCount > 0){
